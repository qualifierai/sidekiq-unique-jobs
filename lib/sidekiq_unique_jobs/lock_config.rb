...
module SidekiqUniqueJobs
  ...
  class LockConfig
    ...
    def on_client_conflict
      @on_client_conflict ||= on_conflict.stringify_keys["client"] if on_conflict.is_a?(Hash)
      @on_client_conflict ||= on_conflict
    end

    # the strategy to use as conflict resolution from sidekiq server
    def on_server_conflict
      @on_server_conflict ||= on_conflict.stringify_keys["server"] if on_conflict.is_a?(Hash)
